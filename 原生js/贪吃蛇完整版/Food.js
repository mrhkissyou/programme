...
(function(window){
    ...
    var newDiv = null;
    Food.prototype.init = function (map,snake) {//创建好的DOM对象放入到参数(map)中;
		...
		var num2 = Math.random();
            this.top = parseInt(num1*(map.offsetHeight-this.height)/this.height)*this.height;
            this.left = parseInt(num2*(map.offsetWidth-this.width)/this.width)*this.width;
	console.log(num1,this.top);
	    var bool = true;
            for(var i=0;i<snake.body.length;i++){
                if(this.top == snake.body[i].top*20 && this.left == snake.body[i].left*20){
                    bool = false;
                }
            }
            if(bool){
